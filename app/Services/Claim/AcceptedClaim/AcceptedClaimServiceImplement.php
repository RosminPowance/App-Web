...
class AcceptedClaimServiceImplement extends Service implements AcceptedClaimService
{
  ...
  public function getDataset($query = null)
  {
    ...
    $beginDate = isset($query['ACCEPTED_DATE']) ? dmYtoYmd($query['ACCEPTED_DATE']) : null;
    $endDate   = isset($query['END_DATE']) ? dmYtoYmd($query['END_DATE']) : null;

    if ($beginDate && ! $endDate)
    {
      $raw = "CONVERT(DATE, SUBSTRING(A.ACCEPTED_DATE,1,LEN(A.ACCEPTED_DATE)-3), 23) >= ?";
    ...
    {
      $beginRaw = "CONVERT(DATE, SUBSTRING(A.ACCEPTED_DATE,1,LEN(A.ACCEPTED_DATE)-3), 23)";
